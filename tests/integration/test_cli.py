...
class TestPublishCommandExtended:
    ...
    def test_publish_docx_with_template(self, runner, jamb_project):
        ...
        try:
            os.chdir(jamb_project)
            result = runner.invoke(cli, ["template", str(assets_dir)], catch_exceptions=False)
            assert result.exit_code == 0
...
class TestPublishTemplateCommand:
    ...
    @pytest.mark.quarto
    def test_publish_template_creates_assets(self, runner, tmp_path):
        """Test that template scaffolds theme.scss and reference.docx."""
        import os
