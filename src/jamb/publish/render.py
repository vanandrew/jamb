...
_TYPST_TEMPLATE_NAME = "template.typ"
...
def render_document(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmpdir = Path(tmp)
        theme = reference_doc = typst_template = None
        ...
        if fmt is OutputFormat.HTML:
            ...
            theme = _THEME_NAME
        elif fmt is OutputFormat.DOCX and template:
            shutil.copyfile(template, tmpdir / _REFERENCE_NAME)
            reference_doc = _REFERENCE_NAME
        elif fmt is OutputFormat.PDF and template:
            shutil.copyfile(template, tmpdir / _TYPST_TEMPLATE_NAME)
            typst_template = _TYPST_TEMPLATE_NAME
